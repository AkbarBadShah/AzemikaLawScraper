...
class Scraper(Base):
    ...
    def save_html(data, file='test'):
        with open(f'logs/{file}.html', 'w') as file:
            file.write(data)
    ...
    def run(self):
        nav_links = self.get_nav_links(BaseURL)
        # self.get_blog_data(nav_links['News'])
        # self.about_firm(nav_links['Our Firm'])
        # self.get_testimonials(nav_links['Testimonials'])
        # self.get_attorneys(nav_links['Attorneys'])
        self.get_practice_areas(nav_links['Practice Areas'])
        # self.get_practice_areas()
        return True
